...
p_informal_trend <- ggplot(ind_plot, aes(fecha)) +
  ...
  labs(title = "La formalidad no acompaña automáticamente al empleo", subtitle = "Composición de la población ocupada · ENCFT · 2014 T3–2026 T1", x = NULL, y = NULL, caption = "Fuente: BCRD, ENCFT · 2026 T1 preliminar · Elaboración: Leonardo Mena") + theme_editorial()
...
p_rama <- ggplot(rama, aes(pct_informal, rama)) + geom_col(fill = pal$terracota, colour = pal$tinta, linewidth = .25, width = .68) + geom_text(aes(label = sprintf("%.1f%%", pct_informal)), hjust = -.12, size = 3.2, fontface = "bold") + scale_x_continuous(labels = label_percent(scale = 1), limits = c(0, 108), breaks = seq(0, 100, 20)) + labs(title = "La informalidad se concentra en ramas distintas", subtitle = "Porcentaje de ocupados informales dentro de cada rama · último trimestre disponible", x = NULL, y = NULL, caption = "Fuente: BCRD, ENCFT · cálculo sobre sector formal e informal · Elaboración: Leonardo Mena") + theme_editorial("x")
...
p_educ <- ggplot(educ, aes(pct_informal, dimension)) + geom_col(fill = pal$oliva, width = .62, colour = pal$tinta, linewidth = .25) + geom_text(aes(label = sprintf("%.1f%%", pct_informal)), hjust = -.12, size = 3.4, fontface = "bold") + scale_x_continuous(labels = label_percent(scale = 1), limits = c(0, 108), breaks = seq(0, 100, 20)) + labs(title = "La educación cambia el tipo de empleo disponible", subtitle = "Informalidad dentro de cada nivel educativo · ENCFT · último trimestre disponible", x = NULL, y = NULL, caption = "Fuente: BCRD, ENCFT · porcentajes calculados con población ocupada formal e informal") + theme_editorial("x")
...
p_waffle <- ggplot(waffle, aes(col, row, fill = estado)) +
  ...
  labs(title = sprintf("%.1f%% de los ocupados son informales", latest_informal$valor[[1]]),
...
p_rd_su4 <- ggplot(rd_regions) + geom_sf(aes(fill = su4), colour = pal$crema, linewidth = .42) +
  ...
  labs(title = "La presión laboral tampoco es uniforme en RD", subtitle = "SU4 por macroregión · escala cromática ajustada al rango observado", caption = sprintf("Fuente: BCRD, ENCFT, corte regional %s · SU4 no es una tasa de informalidad · escala: %.0f–%.0f%%", regional$anio[[1]], su4_limits[[1]], su4_limits[[2]])) +
...
p_scenario_cost <- ggplot(scenario_long, aes(escenario, porcentaje, fill = concepto)) + geom_col(position = position_dodge(width = .78), width = .68, colour = pal$tinta, linewidth = .2) +
  ...
  labs(title = "El costo relativo cae después de los topes de cotización", subtitle = "Costo adicional como porcentaje del salario bruto · 2026 · sin ISR ni cesantía", x = NULL, y = "Costo adicional sobre el salario", caption = "Fuentes: TSS, Resolución 01-2025 e INFOTEP · topes 2026: SFS RD$232,230; pensiones RD$464,460; riesgos RD$92,892") + theme_editorial("none") + theme(axis.text.x = element_text(size = 8.5))
...
p_rem <- ggplot(indexed, aes(fecha, indice, colour = serie)) + geom_line(linewidth = 1) + scale_colour_manual(values = c(remesas_millones_usd = pal$oliva, dops_por_100usd = pal$azul, dops_reales_por_100usd = pal$terracota), labels = c(remesas_millones_usd = "Remesas en USD", dops_por_100usd = "RD$ por US$100", dops_reales_por_100usd = "Poder de compra de US$100")) + scale_x_date(date_breaks = "2 years", date_labels = "%Y") + labs(title = "Una remesa puede valer más pesos y comprar menos", subtitle = "Índice: enero de 2010 = 100 · remesas nominales, tipo de cambio y poder de compra", x = NULL, y = "Índice", caption = "Fuentes: BCRD, remesas, tipo de cambio e IPC nacional · Elaboración: Leonardo Mena") + theme_editorial()
...
p_power <- ggplot(power, aes(fecha, real_vs_nominal)) + geom_hline(yintercept = 100, linetype = "dashed", colour = pal$gris) + geom_line(colour = pal$terracota, linewidth = 1.1) + scale_x_date(date_breaks = "2 years", date_labels = "%Y") + scale_y_continuous(labels = label_percent(scale = 1), breaks = seq(70, 110, 10)) + labs(title = "El tipo de cambio no cuenta toda la historia", subtitle = "Poder de compra real de los pesos recibidos por cada US$100 · enero de 2010 = 100", x = NULL, y = "Poder de compra relativo", caption = "Cálculo: tipo de cambio de compra deflactado por IPC nacional · Fuente: BCRD") + theme_editorial()
...
p_gen <- ggplot(generation, aes(gwh, fuente)) + geom_col(fill = pal$azul, colour = pal$tinta, linewidth = .25, width = .68) + geom_text(aes(label = sprintf("%.0f GWh · %.0f%%", gwh, pct)), hjust = -.08, size = 3.2, fontface = "bold") + scale_x_continuous(labels = label_number(big.mark = "."), limits = c(0, 11000), expand = expansion(mult = c(0, .05))) + labs(title = "La matriz eléctrica es más fósil de lo que suena", subtitle = "Generación acumulada del SENI por combustible · 2024", x = "GWh", y = NULL, caption = "Fuente: MEM, Boletín anual de Generación y Gestión de Energía 2024, p. 3 · Elaboración: Leonardo Mena") + theme_editorial("x")
...
p_loss <- ggplot(ratios, aes(porcentaje, reorder(indicador, porcentaje))) + geom_col(fill = pal$terracota, colour = pal$tinta, width = .62) + geom_text(aes(label = sprintf("%.1f%%", porcentaje)), hjust = -.12, size = 3.4, fontface = "bold") + scale_x_continuous(labels = label_percent(scale = 1), limits = c(0, 108), breaks = seq(0, 100, 20)) + labs(title = "El sistema compra energía que no logra convertir en cobros", subtitle = "Indicadores agregados de las EDE · diciembre de 2024", x = NULL, y = NULL, caption = "Fuente: MEM, Boletín de Distribución y Comercialización, pp. 4–5 · Elaboración: Leonardo Mena") + theme_editorial("x")
...
p_deficit <- ggplot(deficit, aes(valor, concepto)) + geom_col(fill = pal$ocre, colour = pal$tinta, width = .62) + geom_text(aes(label = sprintf("US$ %.1f MM", valor)), hjust = -.08, size = 3.3, fontface = "bold") + scale_x_continuous(labels = label_number(prefix = "US$ ", suffix = " MM"), limits = c(0, 1900), expand = expansion(mult = c(0, .05))) + labs(title = "Las pérdidas de distribución terminan en el espacio fiscal", subtitle = "Déficits acumulados y subsidio reportados por las EDE · 2024", x = NULL, y = NULL, caption = "Fuente: MEM, Boletín de Distribución y Comercialización, p. 7 · Elaboración: Leonardo Mena") + theme_editorial("x")
...
p_electric_map <- ggplot(rd_prov) + geom_sf(aes(fill = perdida_pct), colour = pal$crema, linewidth = .42) +
  ...
  labs(title = "Las pérdidas eléctricas no se reparten igual", subtitle = "Pérdida total reportada por provincia · porcentaje de la energía suministrada", caption = "Fuente: MEM, Informe de Gestión Comercial EDE 2026 · corte mayo 2025–abril 2026 · no separa pérdidas técnicas y no técnicas") +
  theme_editorial("none") + theme(axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), legend.position = "right")
...
p_pollo <- ggplot(pollo_index, aes(fecha)) + geom_line(aes(y = indice_base, colour = producto), linewidth = 1) + geom_line(aes(y = dolar_base, colour = "Dólar"), linewidth = 1, linetype = "dashed") + scale_colour_manual(values = c("Pollo fresco" = pal$terracota, "Pechuga de pollo" = pal$ocre, "Carnes" = pal$oliva, "Dólar" = pal$azul)) + scale_x_date(date_breaks = "6 months", date_labels = "%Y-%m", expand = expansion(mult = c(.01, .04))) + labs(title = "El pollo no se mueve como una simple traducción del dólar", subtitle = "Índices de precios: octubre de 2020 = 100 · pollo fresco, pechuga, carnes y dólar", x = NULL, y = "Índice", caption = "Fuentes: BCRD, IPC por artículos y tipo de cambio de referencia · Elaboración: Leonardo Mena") + theme_editorial()
...
p_yoy <- ggplot(yoy, aes(fecha)) + geom_hline(yintercept = 0, colour = pal$gris, linewidth = .5) + geom_line(aes(y = inflacion_interanual, colour = producto), linewidth = .9) + geom_line(aes(y = dolar_interanual, colour = "Dólar"), linewidth = .9, linetype = "dashed") + scale_colour_manual(values = c("Pollo fresco" = pal$terracota, "Pechuga de pollo" = pal$ocre, "Carnes" = pal$oliva, "Dólar" = pal$azul)) + scale_x_date(date_breaks = "6 months", date_labels = "%Y-%m") + scale_y_continuous(labels = label_percent(scale = 1), breaks = seq(-20, 80, 20)) + labs(title = "Los shocks del pollo tienen vida propia", subtitle = "Variación interanual de precios · comparación con el tipo de cambio", x = NULL, y = "Variación interanual", caption = "Fuente: BCRD, IPC por artículos y mercado cambiario · Elaboración: Leonardo Mena") + theme_editorial()
...
write_csv(problem_data, file.path(mipyme_dir, "data", "procesados", "02_problemas_mipyme.csv")); p_mip_prob <- ggplot(problem_data, aes(tasa, reorder(problema, tasa))) + geom_col(fill = pal$terracota, width = .62, colour = pal$tinta, linewidth = .25) + geom_text(aes(label = sprintf("%.1f%%", tasa)), hjust = -.1, size = 3.2, fontface = "bold") + scale_x_continuous(labels = label_percent(scale = 1), limits = c(0, max(problem_data$tasa) * 1.2)) + labs(title = "La supervivencia tiene más de un enemigo", subtitle = "Problemas declarados por las MIPYMES · porcentaje ponderado", x = NULL, y = NULL, caption = "Fuente: ENMIPYMES 2023 · selección de problemas del cuestionario P5.3 · Elaboración: Leonardo Mena") + theme_editorial("x")
...
p_mip_credit <- ggplot(credit, aes(tamano, tasa, fill = formalidad)) + geom_col(position = position_dodge(width = .78), width = .68, colour = pal$tinta, linewidth = .25) + geom_text(aes(label = sprintf("%.1f%%", tasa)), position = position_dodge(width = .78), vjust = -.45, size = 2.7, fontface = "bold") + scale_fill_manual(values = c(Formal = pal$oliva, Informal = pal$terracota)) + scale_y_continuous(labels = label_percent(scale = 1), limits = c(0, max(credit$tasa, na.rm = TRUE) * 1.25)) + labs(title = "El tamaño sigue importando después del RNC", subtitle = "Acceso a crédito por tamaño y formalidad · ENMIPYMES 2023", x = NULL, y = "Porcentaje de empresas", caption = "Fuente: ENMIPYMES 2023 · estimación ponderada · Elaboración: Leonardo Mena") + theme_editorial()
...
p_tesis_heat <- ggplot(tesis_heat, aes(grupo, problema, fill = tasa)) + geom_tile(colour = pal$crema, linewidth = .7) + geom_text(aes(label = sprintf("%.0f%%", tasa)), size = 2.8, fontface = "bold") + scale_fill_gradient(low = pal$crema, high = pal$terracota, limits = c(0, 100), labels = label_percent(scale = 1), name = "Porcentaje") + labs(title = "Los obstáculos cambian con el tamaño y la formalidad", subtitle = "Problemas declarados por grupo · datos procesados de la tesis MIPYME", x = NULL, y = NULL, caption = "Fuente: ENMIPYMES 2022–2023, archivo reproducible de tesis · estimación ponderada") + theme_editorial("none") + theme(axis.text.x = element_text(angle = 35, hjust = 1, size = 9))
...
p_mipyme_map <- ggplot(rd_prov_mipyme) + geom_sf(aes(fill = formalidad_pct), colour = pal$crema, linewidth = .42) +
  ...
  labs(title = "La formalidad MIPYME también tiene geografía", subtitle = "Porcentaje ponderado de MIPYMES formales por provincia · escala 0–60%", caption = "Fuente: ENMIPYMES 2023 · archivo reproducible de tesis MIPYME · estimación ponderada por factor de expansión") +
  theme_editorial("none") + theme(axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), legend.position = "right")
export_plot(p_mipyme_map, mipyme_fig, "05_mapa_rd_formalidad_mipyme", 8.5, 7.3)
write_chart_map(mipyme_dir, list(tibble(id = "01_ingreso_por_empleado_formalidad", pregunta = "¿Qué carga productiva soporta el emprendedor?", familia = "Barras comparables", fuente = "ENMIPYMES 2023", advertencia = "Promedios ponderados; no es causalidad"), tibble(id = "02_problemas_de_supervivencia_mipyme", pregunta = "¿Qué problemas sostienen la supervivencia?", familia = "Barras ordenadas", fuente = "ENMIPYMES 2023", advertencia = "Respuesta declarada"), tibble(id = "03_credito_formalidad_tamano", pregunta = "¿Qué pesa más, formalidad o tamaño?", familia = "Barras agrupadas", fuente = "ENMIPYMES 2023", advertencia = "No es efecto causal")))
