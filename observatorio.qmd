...
suppressPackageStartupMessages({
  ...
  library(htmltools)
  library(DT)
  library(bslib) # Librería clave para Value Boxes modernos
  library(bsicons)
})
...
# Función robusta para parsear meses en español
...
theme_observatorio <- function() {
  hc_theme(
    ...
    # Eje X con línea guía en Terracota
    xAxis = list(
      gridLineWidth = 0,
      lineColor = "#e0e0e0",
      tickColor = "#e0e0e0",
      ...
      crosshair = list(
        width = 1,
        color = "#c86448", # <--- Terracota para la guía
        dashStyle = "Dash"
      )
    ),
    yAxis = list(
...
# Función para descarga de datos (Optimizada)
generar_boton_descarga <- function(df, nombre_archivo) {
  datatable(
    ...
    options = list(
      dom = 'Bfrtip', 
      buttons = list(list(extend = 'csv', filename = nombre_archivo, text = '📥 Descargar Data', className = 'btn-sm')),
      pageLength = 5,
...
# --- 2. FUNCIONES DE EXTRACCIÓN (Restauradas al estilo original) ---

# DÓLAR
obtener_dolar <- function() {
  tryCatch({
    tf <- tempfile(fileext = ".xlsx")
    # Configuración original simple: solo ignorar SSL
    ...
    message("❌ Error descargando Dólar: ", e$message) # <--- AQUÍ VEREMOS EL ERROR REAL
...
obtener_tpm <- function() {
  tryCatch({
    ...
    # 1. Descarga simple (como en el diagnóstico)
    ...
    raw <- tryCatch(
      ...
      error = function(e) {
         # Si falla, intentamos copiar a extensión .xls por si acaso
    ...
    # 3. Limpieza (Idéntica a tu script de prueba)
    clean <- raw %>%
      select(anio_raw = ...1, mes = ...2, tpm = ...3) %>%
      fill(anio_raw, .direction = "down") %>%
      mutate(
        # Forzamos numéricos para evitar errores de tipo
      ...
      # Ordenamos para el gráfico (Viejo -> Nuevo)
      arrange(fecha) %>%
      select(fecha, tpm)
...
obtener_imae <- function() {
  tryCatch({
    ...
    # Col A=Año, B=Mes, C=Indice, D=Var.Mensual, E=Var.Acumulada
    raw <- read_excel(tf, col_names = FALSE, skip = 6)
    
    clean <- raw %>%
      select(anio_raw = ...1, mes = ...2, var_mensual = ...4, var_acum = ...5) %>%
      # 1. Rellenar Años (El año solo aparece en enero, los demás son NA)
      fill(anio_raw, .direction = "down") %>%
      # 2. Convertir a numéricos para evitar filas de texto basura
      mutate(
        anio = suppressWarnings(as.numeric(anio_raw)),
        var_mensual = suppressWarnings(as.numeric(var_mensual)),
        var_acum = suppressWarnings(as.numeric(var_acum))
      ) %>%
      # 3. FILTRO CRÍTICO: Eliminar filas "Promedio", "Trimestre" o vacías
      filter(
        !is.na(anio), 
        !is.na(mes),
        !str_detect(mes, "Promedio|promedio|Trimestre|Semestre|Total") # <--- Tu petición
      ) %>%
      # 4. Parsear mes (Tu función ya maneja "Enero" -> "ene" -> 1)
      mutate(mes_num = parsear_mes(mes)) %>%
    ...
    if(nrow(clean) == 0) stop("IMAE vacío tras limpieza")
    return(clean)
    
  }, error = function(e) {
    message("❌ Error en IMAE: ", e$message)
    return(data.frame(fecha = Sys.Date(), var_mensual = 0, var_acum = 0))
  })
}

# --- 3. EJECUCIÓN Y CÁLCULOS KPI ---
...
# CÁLCULOS IMAE
val_imae <- tail(df_imae, 1) # Último dato disponible
# Prevención si viene vacío
...
hchart(data_grafico, "line", hcaes(x = fecha, y = tasa_venta), color = "#546e7a", name = "Venta") %>%
  hc_title(text = "Dólar Spot (Últimos 12 meses)") %>%
  hc_yAxis(
    title = list(text = ""),
...
hchart(df_ipc_reciente, "line", hcaes(x = fecha, y = doce_meses), color = "#c86448", name = "Inflación") %>%
  hc_title(text = "Inflación (Últimos 4 años)") %>%
  hc_yAxis(title = list(text = ""),
    plotLines = list(
      ...
      # Línea Meta (Gris oscuro punteado, serio)
      list(
        value = 4.0, 
        color = "#333333", 
        dashStyle = "ShortDash", 
...
```{r}
#| expandable: false
# Preparamos datos: Filtramos últimos 7 años y convertimos a porcentaje
df_tpm_grafico <- df_tpm %>% 
  arrange(desc(fecha)) %>% # Ordenar para head()
  head(84) %>%             # Tomar últimos 84 meses
  arrange(fecha) %>%       # Reordenar cronológico para el gráfico
...
hchart(
  ...
  hc_title(text = "Actividad Económica (IMAE)") %>%
  hc_subtitle(text = "Variación interanual del índice (%)") %>%
  hc_yAxis(
    title = list(text = "Crecimiento %"),
    plotLines = list(list(
      value = 0, 
      color = "#333", 
      width = 2 # Línea base del 0% marcada fuerte
    ))
  ) %>%
  hc_tooltip(
    pointFormat = "Variación Mensual: <b>{point.y:.1f}%</b><br/>"
  ) %>%
  hc_add_theme(theme_observatorio()) |> 
  hc_credits(
    enabled = TRUE, 
...
```{r}
# Botón de descarga pequeño y discreto
